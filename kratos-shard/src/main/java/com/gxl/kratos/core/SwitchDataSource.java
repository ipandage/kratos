...
import com.gxl.kratos.datasource.config.DataSourceHolder;
import com.gxl.kratos.exception.ShardException;
import com.gxl.kratos.shard.DbRule;
import com.gxl.kratos.shard.GetKeyName;
import com.gxl.kratos.shard.TbRule;
import com.gxl.kratos.sql.dialect.mysql.parser.ResolveRoute;
import com.gxl.kratos.sql.dialect.mysql.parser.ResolveTableName;
...
public class SwitchDataSource {
	...
	protected List<Object> manyTbshard(String sql, Object[] params, boolean operation) {
		...
		/* 解析路由条件 */
		long key = resolveRoute.getRoute(sql, keyNames);
		/* 获取数据库索引 */
		...
		String newSql = switchTab.setTabName(dbIndex, tbIndex, dbSize, tbSize, kJdbcTemplate.getConsistent(),
				resolveTableName.getTabName(sql), sql);
	...
	protected List<Object> oneTbshard(String sql, Object[] params, boolean operation) {
		...
		if (kJdbcTemplate.getConsistent()) {
			/* 持有真正的数据库表名的sql */
			newSql = switchTab.setTabName(dbIndex, resolveTableName.getTabName(sql), sql);
		} else {
			newSql = sql;
		}
		/* 获取获取master/slave的数据源启始索引 */
