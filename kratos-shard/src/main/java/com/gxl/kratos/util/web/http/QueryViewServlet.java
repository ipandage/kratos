 ...
 */
package com.gxl.kratos.util.web.http;
...
public class QueryViewServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		KratosJdbcTemplate jdbcTemplate = (KratosJdbcTemplate) context.getBean("kJdbcTemplate");
	...
	protected byte[] initView(String path) {
		...
		try {
			...
			logger.error("无法成功加载到kratos相关的配置文件信息", e);
		} finally {
			if (null != reader) {
				try {
					reader.close();
